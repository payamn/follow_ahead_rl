...
:param check_freq: (int)
    ...
    :param verbose: (int)
    """
    def __init__(self, check_freq: int, log_dir: str, save_frequency=1000, verbose=1):
        super(SaveOnBestTrainingRewardCallback, self).__init__(verbose)
        self.check_freq = check_freq
        self.log_dir = log_dir
        self.save_path = os.path.join(log_dir, 'latest_model')
...
num_agents = 1
env = make_vec_env('gazebo-v0', n_envs=num_agents,  monitor_dir=log_dir)
for i in range (num_agents):
    env.envs[i].set_agent(i)
...
model = model.load("tmp/best_model.zip", env=env)
print ("start training")
model.learn(total_timesteps=25000000, callback=callback)
